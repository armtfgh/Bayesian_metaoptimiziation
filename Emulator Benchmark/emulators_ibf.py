...
class emulators():
    ...
    def xgboost(self):
        regressor = XGBRegressor(alpha=0.4,eta=0.2)
        return regressor 
...
for i in range(len(df['Solvent'])):
    ...
    if df['Buli'][i]==0:
        ...
        df['Buli'][i]='t-BuLi'
n_ensembles = 10
df = pd.get_dummies(df,columns=['Solvent','Buli'],dtype=float)                
numericals= df.iloc[:,:4]
categorials =df.iloc[:,5:]
X = numericals.join(categorials).to_numpy()
y = df['Yield'].to_numpy()
# df1 = X.join(df['Yield'])
